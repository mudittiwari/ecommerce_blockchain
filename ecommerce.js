...
contract('Ecommerce', ([deployer, author, tipper]) => {
    ...
    describe('deployment', () => {
        let product, productcounter, user,usercounter, member, order,prod,dprod,euser,duser,citemadd,deleteorder,ordercounter,removefromcart,removemember,editmember;
        before(async () => {
            ...
            editmember=await ecommerce.editfamilymember('94135655045',"nimit", "nimit2",10,12,"male", { from: author })
            ...
            order = await ecommerce.createorder("mudit",
                ...
                "11:11:2022",
                500,{ from: author })
            deleteorder=await ecommerce.deleteorder(1,{from:author});
            removefromcart=await ecommerce.removefromcart(1,"94135655045",{from:author});
            removemember=await ecommerce.deletefamilymember("94135655045","nimit2",12,{from:author});
            ...
            ordercounter=await ecommerce.ordercounter();
        ...
        it('remove from cart', async () => {
            ...
            let event = removefromcart.logs[0].args;
            assert.equal(event.author, author, 'author is correct');
            assert.equal(event.phone_number, "94135655045", 'phone number is correct');
            assert.equal(event.product_id, 1, 'product id is correct');
        }
        );
        ...
        it('edit family member', async () => {

            // assert.equal(postcounter, 1);
            let event = editmember.logs[0].args;

            assert.equal(event.name, 'nimit2', 'name is correct');
            assert.equal(event.phone_number, '94135655045', 'name is correct');
            assert.equal(event.author, author, 'author is correct');

            assert.equal(event.gender, "male", 'gender is correct');
            assert.equal(event.age, 12, 'age is correct');
        }
        );
        it('remove family member', async () => {

            // assert.equal(postcounter, 1);
            let event = removemember.logs[0].args;

            assert.equal(event.name, 'nimit2', 'name is correct');
            assert.equal(event.author, author, 'author is correct');

            assert.equal(event.phone_number, "94135655045", 'phone is correct');
            assert.equal(event.age, 12, 'age is correct');
        }
        );
        ...
        it('delete order', async () => {
            ...
            // assert.equal(postcounter, 1);
            let event = order.logs[0].args;
            assert.equal(event.id.toNumber(), ordercounter.toNumber() - 1, 'id is correct');
            assert.equal(event.author, author, 'author is correct');
        }
        );
